...
The alert manager exposes no port, so its health is a file whose modification time the
service refreshes only while it is connected to the MQTT broker. The healthcheck runs
`python -m alert_manager.liveness <path>` and gets exit code 0 while the file is fresh.
"""
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m alert_manager.liveness",
